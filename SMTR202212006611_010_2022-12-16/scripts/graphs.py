...
def create_radius_element_map(map_point, color, tooltip, weight = 0.3, fill = True, radius = 500):
    return folium.Circle(
        location = [
            map_point.y,
            map_point.x
...
def create_trip_map(
    ...
    # Gera layout do mapa
    title = f"""
    <h4 align='center' style='font-size:16px'><b>Posições de GPS da viagem</b><br><br>
    ...
    for _, p in posicoes.iterrows():
        ...
        create_radius_element_map(
            ...
            color = status_colors[p.status_viagem],
...
if __name__ == "__main__":
    ...
    # Tabela de posições classificadas
    ## Deve conter as colunas: id_veiculo,servico,timestamp_gps,posicao_veiculo_geo,status_viagem
    posicoes = pd.read_csv("caminho/para/posicoes.csv")

    # Tabela do(s) shape(s)
    ## Deve conter as colunas: shape_id,shape,start_pt,end_pt
    shapes = pd.read_csv("caminho/para/shapes.csv")
    ...
    map.save(f'caminho/para/{title}.html')